...
def run_object_detection(source=0, flip=False, use_popup=False, skip_first_frames=0, model=quantized_det_model,
                         device="AUTO"):
    player = None
    if device != "CPU":
    ...
    quantized_det_compiled_model = core.compile_model(quantized_det_model, "AUTO", ov_config)
    ...
    try:
        ...
        while True:
            ...
            plt.clf()
            sns.kdeplot(data=df, x='x', y='y', fill=True, thresh=0, levels=100, cmap="rocket")
...
if __name__ == '__main__':
    run_object_detection(source=0, flip=True, use_popup=True, model=quantized_det_model, device="AUTO")